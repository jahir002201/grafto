...
<div class="max-w-2xl mx-auto mt-10 bg-white shadow-lg rounded-xl p-8">
    ...
    <form action="{{ route('plants.update', $plant->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <!-- Current Image -->
        <div>
            <label for="current_image" class="block text-sm font-medium text-gray-700 mb-2">
                Current Image
            </label>
            @if($plant->image)
                <img src="{{ Storage::url($plant->image) }}" alt="{{ $plant->name }}" class="w-32 h-32 object-cover rounded-lg">
            @endif
        </div>
        <!-- New Image -->
        <div>
...
-sm font-medium text-gray-700 mb-2">
        ...
        <div>
            ...
            <input
                ...
                name="watering"
                id="watering"
                value="{{ old('watering', $plant->watering) }}"
                class="w-full rounded-lg border border-gray-300 px-4 py-2 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500 outline-none transition"
            >
        </div>
        <!-- Sunlight -->
        <div>
            ...
            <input
                ...
                name="sunlight"
                id="sunlight"
                value="{{ old('sunlight', $plant->sunlight) }}"
                class="w-full rounded-lg border border-gray-300 px-4 py-2 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500 outline-none transition"
            >
        </div>
        <!-- Buttons -->
